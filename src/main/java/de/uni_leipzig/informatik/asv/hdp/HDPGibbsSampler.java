...
public class HDPGibbsSampler {
	...
	public final  double beta = 0.5; // default only
	public final double gamma = 1.5;
	...
	int sampleTable(int docID, int i) {
		...
		double fNew = sampleTable1(k, docState, i, vb);
	...
	private double sampleTable1(int k, DOCState docState, int i, double vb) {
	...
	private double sampleTable2(DOCState docState) {
		double pSum = 0.0;
		for (int j = 0; j < docState.numberOfTables; j++) {
...
//		String corpusFile = "/Volumes/HDD2/ren_data/dev_hdd/bluebrain/9_lda/topic_models_datasets/20_newsgroups/twenty_newsgroups.lda-c";
		String corpusFile = "/Volumes/HDD2/ren_data/dev_hdd/bluebrain/9_lda/topic_models_datasets/pubmed_abstracts_100k.ldac-txtbag";
		...
		for (int d = 0; d < hdp.docStates.length; d++) {
			...
			for (int i = 0; i < docState.documentLength; i++) {
				...
				file.println(docID + " " + docState.words[i].termIndex + " "
						+ docState.tableToTopic[t] + " " + t);
			}
		}
		file.close();
	}
